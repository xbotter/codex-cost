...
impl UsageProvider for CodexUsageProvider {
    ...
    fn collect_daily_usage_series(
        ...
        for path in self.session_files() {
            ...
            for snapshot in parse_session_jsonl_range(&path, &contents, start, end)? {
                let date = local_date_from_rfc3339(&snapshot.timestamp)
                    .context("invalid usage timestamp")?;
                let day_models = per_day_model.entry(date).or_default();
                day_models
                    .entry(snapshot.model_name)
                    .or_default()
                    .add_assign(&snapshot.usage);
...
pub fn parse_session_jsonl(
    ...
    parse_session_jsonl_with_filter(_path, contents, |date| date == target_date)
}

fn parse_session_jsonl_range(
    path: &Path,
    contents: &str,
    start: NaiveDate,
    end: NaiveDate,
) -> Result<Vec<UsageSnapshot>> {
    parse_session_jsonl_with_filter(path, contents, |date| date >= start && date <= end)
}

fn parse_session_jsonl_with_filter<F>(
    _path: &Path,
    contents: &str,
    include_date: F,
) -> Result<Vec<UsageSnapshot>>
